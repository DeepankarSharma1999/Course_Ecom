...
export default async function CourseFaqs({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const course = await prisma.course.findUnique({ where: { id } });
  if (!course) notFound();
  const faqs = await prisma.fAQ.findMany({ where: { courseId: id }, orderBy: { order: "asc" } });
  const createBound = createFaq.bind(null, id);
  return (
    <>
      <AdminTopbar title={`FAQs: ${course.shortTitle || course.title}`} />
      <div className="p-6">
        <PageHeader
          title="FAQs"
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            {faqs.length === 0 && <div className="card p-6 text-center text-ink-500 text-sm">No FAQs yet.</div>}
            {faqs.map((f) => (
              <form key={f.id} action={updateFaq.bind(null, f.id, id)} className="card p-4 space-y-3">
                <div className="grid grid-cols-12 gap-3">
                  <div className="col-span-11">
                    <Input name="question" defaultValue={f.question} className="font-semibold" />
                  </div>
                  <div className="col-span-1">
                    <Input type="number" name="order" defaultValue={f.order} title="Order" />
                  </div>
                </div>
                <Textarea name="answer" rows={3} defaultValue={f.answer} />
                <div className="flex justify-between">
                  <button type="submit" className="btn-outline">Save</button>
                  <button formAction={deleteFaq.bind(null, f.id, id)} className="text-red-600 hover:underline text-sm">Delete</button>
                </div>
              </form>
            ))}
          </div>
