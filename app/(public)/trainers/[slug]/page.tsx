...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: t.bio?.slice(0, 160) ?? `Learn from ${t.name} at Course_Ecom.`,
    openGraph: { title: `${t.name} | Course_Ecom`, description: t.bio?.slice(0, 160) ?? "", images: t.photo ? [t.photo] : [], url: `${SITE.url}/trainers/${slug}` },
  };
}
